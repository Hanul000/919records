...
async function saveFileHandle(file: File, fileHandle?: LocalFileHandle) {
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      transaction.objectStore(RESUME_STORE).put({ key: sessionKey(file), fileHandle } satisfies ResumeRecord);
...
async function serverJson<T>(response: Response): Promise<T> {
  ...
  if (!response.ok || !data) {
    const message = (data as { error?: string } | null)?.error ?? "요청을 처리하지 못했습니다.";
...
async function receivedBytes(sessionUrl: string, size: number) {
  ...
  if (response.status !== 308) throw new Error("업로드 상태를 확인하지 못했습니다.");
  ...
  while (offset < file.size) {
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
      try {
        ...
        if (attempt === 3) throw new Error("인터넷 연결이 복구되면 같은 파일을 다시 선택해 이어 올려 주세요.");
    ...
    if (response.status !== 308) {
      throw new Error("Google Drive가 이 파일을 받지 못했습니다. 같은 파일을 다시 시도해 주세요.");
    }
...
export default function Home() {
  ...
  const overallProgress = totalBytes === 0 ? 0 : Math.round((completedBytes / totalBytes) * 100);
  const currentItem = useMemo(
    () => items.find((item) => item.state === "uploading") ?? null,
    [items],
  );
  const currentFileProgress = currentItem && currentItem.file.size > 0
  ...
  useEffect(() => {
    ...
    async function restoreInterruptedUpload() {
      ...
      try {
        ...
        if (mounted) {
          setNotice("새로고침 전 업로드를 자동으로 다시 열지 못했습니다. 같은 파일을 다시 선택하면 이어서 올릴 수 있어요.");
        }
      }
    }
    ...
    for (const item of items) {
      ...
      try {
        ...
        if (!session) {
          ...
          const data = await serverJson<{ uploadId: string; sessionUrl: string }>(initResponse);
        ...
        const driveFile = (await sendFile(item.file, session.sessionUrl, (uploadedBytes) => {
        ...
        await serverJson(
          await fetch("/api/upload/complete", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ accessCode, uploadId: session.uploadId, fileId: driveFile.id ?? null }),
          }),
        );
        ...
        updateItem(item.id, { state: "complete", uploadedBytes: item.file.size, message: "Drive에 저장됨" });
      } catch (error) {
        hadError = true;
        const message = error instanceof Error ? error.message : "업로드를 완료하지 못했습니다.";
  ...
  async function chooseFiles() {
    type PickerWindow = Window & {
      showOpenFilePicker?: (options: { multiple: boolean }) => Promise<LocalFileHandle[]>;
    };
  ...
  async function onDrop(event: DragEvent<HTMLDivElement>) {
    ...
    const usableHandles = fileHandles.filter((fileHandle): fileHandle is LocalFileHandle => fileHandle?.kind === "file");

    addFiles(droppedFiles, usableHandles.length === droppedFiles.length ? usableHandles : undefined);
  }
      ...
      <div className="mx-auto max-w-[760px]">
        ...
        <section className="overflow-hidden rounded-none border-[3px] border-[#db6f31] bg-white/90 shadow-[0_10px_24px_rgba(46,27,16,0.18),0_30px_96px_rgba(46,27,16,0.42),0_48px_140px_rgba(46,27,16,0.18)]">
          <div className="border-b-4 border-[#db6f31] bg-white/75 px-6 py-8 text-[#32190e] sm:px-10">
            <p className="mb-2 text-sm font-semibold tracking-wide text-[#74300f]">원본 제출</p>
            <h1 className="text-3xl font-bold tracking-[-0.04em] text-[#db6f31] sm:text-4xl">기록물을 올려 주세요</h1>
            <p className="mt-3 max-w-xl text-base leading-7 text-[#54301d]">
              파일은 촬영자의 Drive가 아니라 919-최종-업로드 안의 내 이름 폴더로 바로 전송됩니다.
            </p>
            <p className="mt-1 text-base leading-7 text-[#54301d]">우측 상단 '보관함 보기'에서 본인이 업로드한 목록을 확인할 수 있습니다.</p>
          </div>

          <div className="space-y-7 px-6 py-8 sm:px-10 sm:py-10">
            <div className="grid gap-5 sm:grid-cols-2">
              <label className="block">
                <span className="mb-2 block text-sm font-semibold text-slate-800">이름</span>
                <input
                  value={name}
            ...
            <div
              ...
              onKeyDown={(event) => {
                if (!isSending && (event.key === "Enter" || event.key === " ")) {
                  event.preventDefault();
                  void chooseFiles();
                }
              }}
            ...
            >
              ...
              <p className="text-lg font-bold tracking-tight text-slate-900">원본 파일을 끌어놓거나 선택하세요</p>
              <p className="mt-2 text-sm leading-6 text-slate-600">영상·사진 등 여러 파일을 선택할 수 있어요. 파일당 최대 100GB.</p>
              <input ref={fileInputRef} onChange={onFileChange} type="file" multiple className="sr-only" />
            </div>

            {items.length > 0 && (
              <section aria-live="polite" className="rounded-none border border-slate-200 bg-slate-50/70 p-4 sm:p-5">
                <div className="mb-4 flex items-baseline justify-between gap-4">
                  <p className="font-bold tracking-tight text-slate-900">선택한 파일 {items.length}개</p>
                  <p className="text-sm font-medium text-slate-600">총 {formatBytes(totalBytes)}</p>
                </div>
                <div className={items.length > 10 ? "max-h-[720px] space-y-3 overflow-y-auto pr-2 [scrollbar-gutter:stable]" : "space-y-3"}>
                  {items.map((item) => {
                    const itemProgress = item.file.size === 0 ? 0 : Math.round((item.uploadedBytes / item.file.size) * 100);
                    return (
                      <article key={item.id} className="rounded-none border border-slate-200 bg-white p-3.5">
                        <div className="flex min-w-0 items-start gap-3">
                          <FileVideo className="mt-0.5 shrink-0 text-[#db6f31]" size={20} aria-hidden="true" />
                          <div className="min-w-0 flex-1">
                            <div className="flex gap-3">
                              <p className="min-w-0 flex-1 truncate text-sm font-semibold text-slate-900">{item.file.name}</p>
                              <p className="shrink-0 text-sm text-slate-500">{formatBytes(item.file.size)}</p>
                            </div>
                            {item.state !== "waiting" && (
                              <>
                                <div className="mt-2 h-1.5 overflow-hidden rounded-none bg-slate-100">
                                  <div className="h-full rounded-none bg-[#db6f31] transition-[width] duration-300" style={{ width: `${itemProgress}%` }} />
                                </div>
                                <p className={`mt-1.5 text-xs ${item.state === "error" ? "text-rose-700" : item.state === "complete" ? "text-emerald-700" : "text-slate-600"}`}>
                                  {item.state === "uploading" ? `${itemProgress}% · ${item.message}` : item.message}
                                </p>
                              </>
                            )}
                          </div>
                          {item.state === "complete" && <CheckCircle2 className="shrink-0 text-emerald-600" size={20} aria-label="완료" />}
                          {item.state === "uploading" && <LoaderCircle className="shrink-0 animate-spin text-[#db6f31]" size={20} aria-label="업로드 중" />}
                        </div>
                      </article>
                    );
                  })}
            ...
            {notice && <p role="alert" className="rounded-none bg-rose-50 px-4 py-3 text-sm font-medium text-rose-800">{notice}</p>}

            {items.length > 0 && (
              <div>
                {isSending && (
                  <section aria-live="polite" className="mb-4 space-y-4 rounded-none border border-[#ecd6c8] bg-[#fff9f5] p-4">
                    <div>
                      <div className="flex items-baseline justify-between gap-4 text-sm">
                        <div className="min-w-0">
                          <p className="font-bold text-slate-800">현재 파일 업로드 진행률</p>
                          <p className="mt-0.5 truncate text-xs text-slate-600" title={currentItem?.file.name}>
                            {currentItem ? currentItem.file.name : "다음 파일을 준비하는 중…"}
                          </p>
                        </div>
                        <span className="shrink-0 font-bold text-[#a84b19]">{currentFileProgress}%</span>
                      </div>
                      <div className="mt-2 h-2 overflow-hidden rounded-none bg-[#f4d9c9]">
                        <div className="h-full rounded-none bg-[#db6f31] transition-[width] duration-300" style={{ width: `${currentFileProgress}%` }} />
                      </div>
                      {currentItem && (
                        <p className="mt-1.5 text-xs text-slate-600">
                          {formatBytes(currentItem.uploadedBytes)} / {formatBytes(currentItem.file.size)}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="border-t border-[#eedbd0] pt-4">
                      <div className="flex items-baseline justify-between gap-4 text-sm">
                        <div>
                          <p className="font-bold text-slate-800">전체 업로드 진행률</p>
                          <p className="mt-0.5 text-xs text-slate-600">
                            {items.length}개 중 {completedFileCount}개 저장됨 · {formatBytes(completedBytes)} / {formatBytes(totalBytes)}
                          </p>
                        </div>
                        <span className="shrink-0 font-bold text-[#a84b19]">{overallProgress}%</span>
                      </div>
                      <div className="mt-2 h-2 overflow-hidden rounded-none bg-[#f4d9c9]">
                        <div className="h-full rounded-none bg-[#db6f31] transition-[width] duration-300" style={{ width: `${overallProgress}%` }} />
                      </div>
                    </div>
                  </section>
                )}
                ...
                >
                  {isSending ? <LoaderCircle className="animate-spin" size={20} aria-hidden="true" /> : <UploadCloud size={20} aria-hidden="true" />}
            ...
            <div className="grid gap-3 border-t border-slate-100 pt-6 text-sm leading-6 text-slate-600 sm:grid-cols-2">
              <p className="flex gap-2"><ShieldCheck className="mt-0.5 shrink-0 text-[#db6f31]" size={18} aria-hidden="true" /><span>내 Drive에 파일이 복사되거나 저장되지 않습니다.<br />보관함은 보기 전용입니다.</span></p>
              <p className="flex gap-2"><WifiOff className="mt-0.5 shrink-0 text-[#db6f31]" size={18} aria-hidden="true" /><span>전송 중에는 이 창을 열어 두세요.<br />연결이 잠시 끊겨도 자동으로 다시 시도합니다.</span></p>
            </div>
          </div>
        </section>
      </div>
